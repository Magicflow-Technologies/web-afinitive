...
function FooterLink({ item }: { item: FooterNavigationLink }) {
  if (!item.href) {
    return (
      <span aria-disabled="true" className="site-footer-link text-sm leading-7">
        {item.label}
      </span>
    );
  }
  ...
  return (
    <Link
      href={item.href}
      className="site-footer-link text-sm leading-7 transition-colors duration-200 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent-muted"
    >
      {item.label}
    </Link>
  );
...
export function SiteFooter() {
  ...
  return (
    <footer className="site-footer [--text-charcoal:var(--color-heading)] [--text-muted:var(--color-body)]">
      <div className="site-footer-inner mx-auto grid w-full max-w-[96rem] gap-12 px-5 py-14 sm:px-8 lg:grid-cols-[minmax(16rem,0.78fr)_minmax(0,1.22fr)] lg:gap-20 lg:px-12 lg:py-22">
        <div className="site-footer-brand flex flex-col justify-start">
          {hasLogo ? (
            <Image
              ...
              width={264}
              height={68}
              className="site-footer-logo h-auto w-[208px] sm:w-[236px] lg:w-[264px]"
            />
          ) : (
        ...
        <div className="site-footer-content grid gap-12 xl:grid-cols-[minmax(0,1fr)_minmax(15rem,0.72fr)] xl:gap-14">
          <nav
            aria-label="Navegación del pie de página"
            className="site-footer-navigation grid gap-x-10 gap-y-10 sm:grid-cols-2 xl:grid-cols-3"
          >
            {footerNavigation.map((category) => (
              <div key={category.title} className="space-y-4">
                <h2 className="site-footer-heading text-[0.82rem] tracking-[0.16em] uppercase">
                  {category.title}
                </h2>
